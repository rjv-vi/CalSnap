// ═══════════════════════════════════════════════════
// CALSNAP SERVICE WORKER — ЧИСТАЯ ВЕРСИЯ
// БЕЗ clients.claim() — именно он вызывал бесконечный сплэш
// ═══════════════════════════════════════════════════
...
self.addEventListener('install', e => {
  // skipWaiting — активируемся сразу
  // НО без clients.claim() — не берём контроль над текущей страницей
  // Контроль получим только при следующем открытии → нет controllerchange → нет loop
  self.skipWaiting();
  e.waitUntil(
...
self.addEventListener('activate', e => {
  // Удаляем старые кэши
  // clients.claim() НАМЕРЕННО УБРАН — он вызывал controllerchange → reload → бесконечный сплэш
  e.waitUntil(
...
self.addEventListener('fetch', e => {
  ...
  // HTML и manifest — всегда сеть (чтобы обновления приходили сразу)
